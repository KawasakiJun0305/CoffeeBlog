...
【事実確認ルール】
- 科学・宇宙・医療・歴史の具体的事実は広く知られた定説のみ記述する
- 組織・機関・固有名詞は正確に使用する（例: ESAの案件をNASAに帰属させない、ISSpressoはESA/イタリア製）
- 不確かな事実は「〜とされています」「〜という研究があります」など出典の性格を示す表現にとどめる
- ニュースの内容を記事内で言及する場合、「BBC報道によると」「今週のニュースで伝えられた通り」など出典を本文に自然に組み込む

【禁止事項】
- 「〜かもしれません」「〜のようです」などの曖昧な逃げ表現
- 「様々な種類があります」などの具体性ゼロの記述
- 根拠のない健康効果の断定
- ニュース・書籍の内容を、出典を伏せたまま断定的に記述すること
...
async function main(): Promise<void> {
  ...
  // Fallback: pull title from first markdown heading if GPT skipped the meta block
  const headingMatch = fullText.match(/^#{1,4}\s+(.{4,30})/m);

  const stripMarkers = (s: string) => s.replace(/^\*+\s*/, "").replace(/\s*\*+$/, "").trim();

  const inspiration = stripMarkers(inspirationMatch?.[1]?.trim() ?? "最新ニュース");
  const category = stripMarkers(categoryMatch?.[1]?.trim() ?? "時事・トレンド");
  const title = stripMarkers(titleMatch?.[1]?.trim() ?? headingMatch?.[1]?.trim() ?? "記事");
